    ...
    public class ASP_AudioWaveformEditor : Editor
    {
        ...
        private void SaveMarkerProfile()
        {
            ...
            if (!string.IsNullOrEmpty(savePath))
            {
                try
                {
                    var profile = ScriptableObject.CreateInstance<MarkerProfile>();
        ...
        private void LoadMarkerProfile()
        {
            ...
            if (!string.IsNullOrEmpty(loadPath))
            {
                ...
                var profile = AssetDatabase.LoadAssetAtPath<MarkerProfile>(loadPath);
        ...
        private ASP_Marker DuplicateMarker(ASP_Marker original, AudioSourcePlus audioSourcePlus)
        {
            ...
            {
                normalizedTimelinePosition = original.normalizedTimelinePosition,
                IsSelected = original.IsSelected,
                IsTriggered = original.IsTriggered,
                MarkerName = original.MarkerName,
                IsEffectActive = original.IsEffectActive,
                EffectDuration = original.EffectDuration,
                justTriggered = original.justTriggered,
                justTriggeredColor = original.justTriggeredColor,
                ExecuteInEditMode = original.ExecuteInEditMode,
                DynamicPickers = new List<ASP_DynamicPicker>(original.DynamicPickers)
            };
            ...
            foreach (var marker in markers)
            {
                if (marker.IsSelected)
                {
                    ...
                    if (GUILayout.Button("Add Event", GUILayout.Height(20)))
                    {
                        ...
                        marker.DynamicPickers.Add(new ASP_DynamicPicker());
