    ...
    public class CozyWeatherEditor : Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            if (weatherSphere.GetModule(out CozySatelliteModule module))
            {
                ...
                float dec = moon.declination * Mathf.Sin(Mathf.PI * 2 * ((weatherSphere.modifiedDayPercentage + (float)(weatherSphere.timeModule.currentDay + moon.rotationPeriodOffset + weatherSphere.timeModule.GetDaysPerYear() * weatherSphere.timeModule.currentYear) % moon.declinationPeriod) / moon.declinationPeriod));
